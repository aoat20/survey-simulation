...
class LoggerBMT:
    ...
    def log_vessel(self,
        ...
        self.log_dict[self.n]['vessels'].append({'vessel_type': vessel.vessel_type,
                                                 ...
                                                 'range_yd': vessel.range_yds,
                                                 'cpa_yd': vessel.cpa_yds,
                                                 'tcpa_s': vessel.tcpa_s,
        ...
        for v in self.log_dict[n]['vessels']:
            if v['vessel_type'] == 'agent':
                ...
                for d_n in self.log_dict[0:n]:
                    agent.xy_hist = np.append(agent.xy_hist,
                                              [v_n['xy']for v_n in d_n['vessels']
                                               if v_n['vessel_type'] == 'agent'],
                                              axis=0)
                agent.xy_hist = agent.xy_hist[1:]
            else:
